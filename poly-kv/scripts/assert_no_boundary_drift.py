...
for cargo in [Path('crates/quant-codec-core/Cargo.toml'), Path('crates/poly-kv/Cargo.toml')]:
    if cargo.exists():
...
# poly-kv must not contain local Turbo/Fib algorithm implementation names beyond adapter stubs/docs.
for p in Path('crates/poly-kv/src').rglob('*.rs') if Path('crates/poly-kv/src').exists() else []:

...
def latest_run_id() -> str:
    runs = sorted(Path(".codex-runs").glob("*-poly-kv-next"))
    if not runs:
        raise SystemExit("no .codex-runs/*-poly-kv-next directory found")
    return runs[-1].name
...
def main() -> int:
    ...
    cmd = [
        sys.executable,
        "-c",
        "import poly_kv, json; print(json.dumps({'native_available': poly_kv.native_available()}))",
    ]
    ...
    result = {
        ...
        "skip_reason": None
        if native_available
        else "poly_kv._native is not built; maturin unavailable or develop/build not run",

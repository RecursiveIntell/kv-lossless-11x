...
REPO_FILES = {
    ...
    "poly-kv-generic-rust-next-codex-context-20260520.report.md": "# old report\n",
    "poly-kv-generic-rust-next-codex-context-20260520.manifest.json": "{}\n",
    "poly-kv-generic-rust-next-codex-context-20260520.zip": "fake zip bytes\n",
}
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        cmd = [
            ...
            "--archive-root-markdown-noise", "--output", "poly-kv-generic-rust-next-codex-context-20990101.zip",
        ]
        result = subprocess.run(cmd, cwd=root, text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        if result.returncode != 0:
            print(result.stdout)
            return result.returncode
        forbidden = ["README_BUNDLE.md", "BUNDLE_MANIFEST.json", "poly-kv-generic-rust-next-codex-context-20260520.report.md"]
        still = [f for f in forbidden if (root / f).exists()]
        if still:
            print("root artifacts were not archived:", still)
            return 1
        manifest = root / "poly-kv-generic-rust-next-codex-context-20990101.manifest.json"
        payload = json.loads(manifest.read_text(encoding="utf-8"))
        paths = {item["path"] for item in payload.get("files", [])}
        required = {"python/poly_kv/_native.pyi", "python/poly_kv/py.typed", ".codex-runs/current/commands_run.log"}
        missing = sorted(required - paths)
        if missing:
            print("manifest missing required paths:", missing)
            return 1
        with zipfile.ZipFile(root / "poly-kv-generic-rust-next-codex-context-20990101.zip") as zf:
